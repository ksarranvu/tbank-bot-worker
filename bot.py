...
MAIN_API = os.getenv("MAIN_API", "https://tbank-bot-production.up.railway.app")
WEBAPP_URL = os.getenv("WEBAPP_URL", "https://ksarranvu.github.io/tbank-bot-worker/")
...
def api_my_stats():
    ...
    try:
        total, completed = get_stats_for_staff(int(staff_id))
    except:
        return jsonify({"error": "bad user_id"}), 400
...
def my_stats(message):
    total, completed = get_stats_for_staff(message.from_user.id)
    bot.send_message(
        message.chat.id,
